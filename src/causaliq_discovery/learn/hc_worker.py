...
from causaliq_core.utils import EnumWithAttrs, values_same
...
class HCWorker:
    ...
    score_cache: Dict[Any, Any] = {}  # cache, shared across workers
    ...
    def __init__(
        ...
        self.params["zero"] = (
            0.0 if isinstance(self.data, Pandas) else self.data.N * 10**-6
        )

        # Initialise parents to represent empty DAG and compute its score.

        self.parents: Dict[str, Set[str]] = {
            n: set() for n in self.data.get_order()
        }
        # print('\nStart scoring empty graph ...')
        self.score = sum(
            [
                HCWorker.nscore(n, set(), self.data, self.params)[0]
        ...
        if self.knowledge is not False and self.knowledge.initial is not None:
            for arc in self.knowledge.initial.edges:
                ...
                self._apply_change_to_dag(
                    DAGChange(GraphAction.ADD, arc, 0.0, {})
                )
            print("\nInitial parents: {}\n".format(self.parents))
        ...
        self.best: Optional[BestDAGChanges] = None
        self.paused: Optional[bool] = None

    def run(self) -> "HCWorker":
        """
        ...
        while self.best.top is None or self.best.top.activity not in {
            GraphAction.STOP,
            GraphAction.PAUSE,
        }:
            ...
            if self.knowledge is not False:
                self.best, know_event = self.knowledge.hc_best(
                    self.best, 6, self.data, self.parents
                )
            else:
                know_event = None
            ...
            if (
                self.best.top.activity  # type: ignore[union-attr]
                not in {GraphAction.NONE, GraphAction.PAUSE}
                and self.best.top.delta is not None  # type: ignore[union-attr]
            ):
                self.score += self.best.top.delta  # type: ignore[union-attr]
                if self.best.top.delta <= 0.0:  # type: ignore[union-attr]
                    self.num_noinc += 1
                    if debug:
                        print(
                            "{} iterations without improvement".format(
                                self.num_noinc
                            )
                        )
            ...
            if self.best.top.activity not in {  # type: ignore[union-attr]
                ...
                self._apply_change_to_dag(self.best.top)  # type: ignore
                if self.tabulist is not None:
                    self.tabulist.add(self.parents)
                ...
                if self.score > self.best_parents_score:
                    ...
                    if (
                        "bnlearn" in self.params
                        and self.params["bnlearn"] is True
                        and not values_same(
                            self.score, self.best_parents_score, sf=10
                        )
                    ):
                        self.num_noinc = 0
                        if debug:
                ...
                if debug:
                    print(
                        (
                            "    change at iter {} ({}) is {} of {} with "
                            + "delta {:.3f} and total score {:.3f}\n"
                        ).format(
                            self.iter,
                            asctime(localtime()),
                            self.best.top.activity.value,  # type: ignore
                            self.best.top.arc,  # type: ignore[union-attr]
                            self.best.top.delta / self.data.N,  # type: ignore
                            self.score / self.data.N,
                        )
                    )

            elif self.best.top.activity == GraphAction.STOP:  # type: ignore
                self.parents = deepcopy(self.best_parents)
                self.score = self.best_parents_score

            self.score2 = round(self.score, 10)
            if self.best.top.activity == GraphAction.PAUSE:  # type: ignore
                self.iter -= 1
                self.paused = True
    ...
    def clone(
        ...
        if sequence is not None or pause is not None:
            ...
            _seq = (
                sequence if sequence is not None else _clone.knowledge.sequence
            )
            _pause = pause if pause is not None else _clone.knowledge.pause
            _clone.knowledge.set_sequence(_seq, _pause)

        return _clone

    @classmethod
    def init_score_cache(self) -> None:
        """
    ...
    def _check_possible_arc_changes_against_best(
        self, arc: Tuple[str, str], delta: List[Any]
    ) -> None:
        """
        ...
        if len(delta) == 1:
            ...
            delete: Optional[DAGChange] = DAGChange(
                GraphAction.DEL, arc, delta[0][0], delta[0][1]
            )
            ...
            reverse: Optional[DAGChange] = DAGChange(
                GraphAction.REV, arc, delta[1][0], delta[1][1]
            )
            ...
            if delete is not None and reverse is not None:
                top = (
                    ...
                    or values_same(delta[0][0], delta[1][0])
                    else reverse
                )
        ...
        if top is not None:
            if self._is_better(top, best.top, self.params["prefer"]) and (
                (self.tabulist is None and top.delta > self.params["zero"])
                ...
                best.second = (
                    best.top
                    if best.top.activity != GraphAction.STOP  # type: ignore
                    else best.second
                )
                if second is not None and (
                    best.second is None
                    or second.delta > best.second.delta  # type: ignore
                ):
                    best.second = second
                best.top = top

            elif (
                self._is_better(top, best.second, self.params["prefer"])
    ...
    def _change_permitted(self, change: DAGChange) -> Optional[DAGChange]:
        """
        ...
        if activity in [GraphAction.ADD, GraphAction.REV]:
            ...
            partial_order = DAG.partial_order(
                self.parents, new_arc=new_arc  # type: ignore[arg-type]
            )
        ...
        if self.tabulist is not None and self.tabulist.hit(
            self.parents,
            change,
            trace=self._is_better(
                change, self.best.top, self.params["prefer"]
            ),
        ):
            return None
        ...
        if self.knowledge is not False and self.knowledge.blocked(
            change,
            (
                True
                if self.best.top.delta is None  # type: ignore[union-attr]
                or (change.delta > self.best.top.delta)  # type: ignore
                else False
            ),
        ):
            return None
    ...
    def _is_better(
        ...
        if (
            prefer in [Prefer.CONN, Prefer.UNCO]
            ...
            and (
                (
                    ...
                    and not _connected(proposed.arc)
                )
                or (
                    prefer == Prefer.UNCO
            ...
            print(
                "*** Reject {}connected arc {}".format(
                    ("dis" if prefer == Prefer.CONN else ""), proposed.arc
                )
            )
            return False
        ...
        draw = (
            values_same(proposed.delta, current.delta, sf=6)  # type: ignore
            if proposed is not None
            and current is not None
            and current.delta is not None
            else False
        )
        ...
        better = not draw and (
            proposed.delta > current.delta  # type: ignore[operator]
        )
        return better or (
            draw and proposed.activity.priority > current.activity.priority
        )

    def _apply_change_to_dag(self, change: DAGChange) -> None:
        """
    ...
    def _update_deltas(self, target: str, inbound: bool) -> None:
        """
        ...
        for node in self.parents.keys():
            ...
            if (
                node == target
                or not len(self.deltas[arc])
                or (inbound is False and len(self.deltas[arc]) == 1)
            ):
                continue
            ...
            if len(self.deltas[arc]) == 1:
                ...
                rev_counts = {
                    k: 0.5 * (del_delta[1][k] + add_delta[1][k])
                    for k in del_delta[1].keys()
                }
                rev_delta = (
                    (
                        0.0
                        if self.parents[arc[1]] - {arc[0]}
                        == self.parents[arc[0]]
                        else del_delta[0] + add_delta[0]
                    ),
                    rev_counts,
                )
    ...
    def _delta(self, arc: Tuple[str, str]) -> Tuple[float, Dict[str, Any]]:
        """
        ...
        before = HCWorker.nscore(
            arc[1], self.parents[arc[1]], self.data, self.params
        )
        ...
        counts = {
            k: 0.5 * (before[1][k] + after[1][k]) for k in before[1].keys()
        }

        return (round(after[0] - before[0], 10), counts)

    def _score(
        self, node: str, parents: Set[str]
        ...
        _parents = {node: list(parents)} if len(parents) else {}
        if key not in HCWorker.score_cache:
            # action = 'MISS'
            _params = dict(self.params)
            scores = [_params.pop("score")]
            score, counts = node_score(
                node, _parents, scores, _params, self.data, counts_reqd=True
            )
            HCWorker.score_cache.update(
                {key: (score[self.params["score"]], counts)}
            )
        # else:
        # action = ' HIT'
        ...
        return HCWorker.score_cache[key]  # type: ignore[no-any-return]
    ...
    def nscore(
        ...
        if key not in HCWorker.score_cache:
            ...
            scores = [_params.pop("score")]
            score, counts = node_score(
                node, _parents, scores, _params, data, counts_reqd=True
            )
            HCWorker.score_cache.update(
                {key: (score[params["score"]], counts)}
            )
        # else:
        # action = ' HIT'
    ...
    def _init_trace(self, context: Optional[Dict[str, Any]]) -> None:
        """
        ...
        if context is None:
            ...
            if self.knowledge is not False:
                context.update(
                    {
                        "knowledge": self.knowledge.label,
                        "initial": self.knowledge.initial,
                    }
                )
            self.trace = Trace(context)

            blocked: Optional[List[Any]] = (
                None if self.tabulist is None else []
            )
            self.trace.add(
                GraphAction.INIT,
                {
                    GraphActionDetail.DELTA: self.score,
                    GraphActionDetail.BLOCKED: blocked,
                },
            )

    def _add_trace_entry(self, know_event: Any) -> None:
        """
        ...
        """
        assert self.best is not None
        assert self.best.top is not None
        assert self.trace is not None

        def _rounded_count(key: str) -> float:
            return round(self.best.top.counts[key], 1)  # type: ignore
        ...
        if self.best.second is not None:
            details.update(
                {
                    GraphActionDetail.ACTIVITY_2: (
                        self.best.second.activity.value
                    ),
        ...
        if self.best.top.counts is not None and "min" in self.best.top.counts:
            details.update(
                {
                    GraphActionDetail.MIN_N: _rounded_count("min"),
        ...
        blocked = (
            [
                e
                for e in self.tabulist.blocked()
                if e[2] > 0
                or (
                    details[GraphActionDetail.DELTA] <= 0.0  # type: ignore
                    and self.num_noinc <= len(self.tabulist.tabu)
                )
            ]
            if self.tabulist is not None
            else None
        ...
        know_event = (
            None
            if know_event is None
            else (
                know_event.rule.value,
                know_event.correct,
                know_event.outcome.value,
                know_event.arc,
            )
        )
