...
KNOWLEDGE_PARAMS = {
    "limit": (int, float, bool),  # limit on AL requests
...
class KnowledgeParams:
    ...
    @classmethod
    def check(self, params: Any, rules: Any, sample: Any) -> None:
        """
        ...
        for param, type in params.items():
            if not isinstance(type, KNOWLEDGE_PARAMS[param]):  # type: ignore
        ...
        if "limit" in params and params["limit"] is not False:
            ...
            if isinstance(limit, float):
                ...
                params["limit"] = max(
                    1, round(limit * len(params["ref"].dag.nodes))
                )
        ...
        if (
            (
                ("stop" in params and not isinstance(params["stop"], dict))
                or ("reqd" in params and not isinstance(params["reqd"], dict))
            )
            and "expertise" not in params
            and "ref" in params
        ):
            print("\n\n** set expertise\n")
        ...
        if (
            ("ignore" in params and params["ignore"] < 0)
            or (
                "expertise" in params
            ...
            or (
                "initial" in params
                and isinstance(params["initial"], bool)
                and params["initial"] is True
            )
            or (
                "threshold" in params
                and (params["threshold"] < 0.0 or params["threshold"] > 1.0)
            )
        ):
            raise ValueError("Knowledge(): bad parameter value")
        ...
        if (
            len(set(rules.rules) & {Rule.EQUIV_ADD, Rule.BIC_UNSTABLE})
    ...
    @classmethod
    def equiv_seq(self, params: Any) -> None:
        """
        ...
        if not len(sequence) or any(
            [not isinstance(v, bool) for v in sequence]
        ):
            raise TypeError("Knowledge(): bad sequence element type")

    @classmethod
    def tiers(self, params: Any, sample: Any) -> None:
        """
        ...
        if (
            ...
            and (num_nodes < 2 or num_nodes > len(ref.dag.nodes))
            or (
                isinstance(num_nodes, float)
                and (num_nodes <= 0.0 or num_nodes > 1.0)
            )
        ):
            raise ValueError("Knowledge(): bad parameter values")
        ...
        nodes = [
            ref.dag.nodes[i]
            for i in RandomIntegers(len(ref.dag.nodes), sample)
        ][:num_nodes]
        ...
        for group in order:
            ...
            for node in group:
                for a in ancestors:
                    ...
                    params["stop"].update(
                        {
                            (node, a): (
                                False if (node, a) in ref.dag.edges else True
                            )
                        }
                    )
            ...
            for p in permutations(group, 2):
                ...
                params["stop"].update(
                    {p: (False if p in ref.dag.edges else True)}
                )

            ancestors = ancestors | group  # nodes in higher tiers

    @classmethod
    def check_arc_params(self, type: Any, params: Any, sample: Any) -> None:
        """
        ...
        if type not in params:
            raise ValueError("Knowledge(): {} param missing".format(type))
    ...
    @classmethod
    def setup_arc_lists(self, type: Any, params: Any) -> None:
        """
        ...
        max_reqd = round(
            len(right)
            if "expertise" not in params
            else params["expertise"] * len(right)
        ...
        while len(params[type]) < num_reqd:
            ...
            print(
                "{}orrect elem {}: {}".format(
                    ("C" if correct else "Inc"), elem, _from[elem]
                )
            )
