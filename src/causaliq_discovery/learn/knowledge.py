...
from causaliq_core.utils.random import init_stable_random, stable_random
...
class Knowledge:
    ...
    def __init__(
        self, rules: Any, params: Any = None, sample: int = 0
    ) -> None:

        if (
            not isinstance(rules, RuleSet)
        ...
        self.expertise = params["expertise"] if "expertise" in params else 1.0
        ...
        self.threshold = params["threshold"] if "threshold" in params else 0.05
        self.count = 0
        self.stop = (
            {}
        ...
        self.reqd = (
            {}
            if "reqd" not in params
            else {a: (v, True) for a, v in params["reqd"].items()}
        )
        self.event: KnowledgeEvent | None = None
        self.event_delta = None
        self.max_abs_delta: float | None = None
        self.initial = (
            params["initial"]
            if "initial" in params and params["initial"] is not False
            else None
        )
    ...
    def set_sequence(self, sequence: Any, pause: bool) -> None:
        """
    ...
    def _label(self, rules: Any) -> str:
        ...
        if rules == RuleSet.EQUIV_ADD:
            label = "limit {}, ignore {}, partial {} and expertise {}".format(
                self.limit, self.ignore, self.partial, round(self.expertise, 3)
            )
        elif rules == RuleSet.BIC_UNSTABLE:
            label = (
                "limit {}, threshold {}, partial {} and expertise {}".format(
                    self.limit,
            ...
            label = "{} prohibited and expertise {}".format(
                len(self.stop), round(self.expertise, 3)
            )
        elif rules == RuleSet.REQD_ARC:
            label = "{} required and expertise {}".format(
                len(self.reqd), round(self.expertise, 3)
            )
            ...
            label = "sequence of length {} then {}pause".format(
                len(self.sequence), "" if self.pause is True else "no "
            )
        else:
            label = "no parameters"
    ...
    def get_arc_knowledge(self, arc: Any, trigger: Any) -> Any:
        """
        ...
        expertise = (
            (
                1.0
                if self.count <= round(0.5 * self.limit)
                else max(2 * self.expertise - 1.0, 0.0)
            )
            if self.earlyok is True
            else self.expertise
        )
        ...
        print(
            "   --- Expert {}correctly says arc is {}".format(
                ("" if correct is True else "in"), answer
            )
        )
    ...
    def new_best(
        self, trigger: Any, know_arc: Any, correct: Any, best: Any
    ) -> Any:
        """
        ...
        if (
            (activity == GraphAction.ADD and know_arc == arc)
            ...
            print(
                "   --- Aborting {} of {}".format(
                    best.top.activity, best.top.arc
                )
            )
            assert new_best.top is not None
    ...
    def hc_best(self, best: Any, sf: Any, data: Any, parents: Any) -> Any:
        """
        ...
        """
        trigger = self.rules.match(
            best.top, best.second, sf, data, parents, self
        )
        ...
        if trigger is not None and not (
            ...
            if trigger == Rule.EQUIV_SEQ:
                ...
                self.count += 1
                print(
                    "\nKnowledge request {} for {} of {} {}".format(
                        self.count,
                        best.top.activity,
                        best.top.arc,
                        (
                ...
                know_arc, correct = self.get_arc_knowledge(
                    best.top.arc, trigger
                )
                ...
                print(
                    ".. {} {}correctly".format(
                        outcome.value, "" if correct is True else "in"
                    )
                )
            ...
            event = (
                self.event  # type: ignore[assignment]
                if self.event is not None
                and (
                    best.top.delta is None
                    or self.event_delta >= best.top.delta
                )
                else None
            )
        ...
        if (
            ...
            and best.top.delta is not None
            and (
                self.max_abs_delta is None
    ...
    def blocked(self, change: Any, trace_event: bool = True) -> Any:
        """
