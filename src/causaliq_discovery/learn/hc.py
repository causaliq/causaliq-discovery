...
from causaliq_data import Data, NumPy
from causaliq_data.pandas import Pandas
from causaliq_data.score import SCORE_PARAMS, check_score_params
...
def _validate_tabu_params(params: Any) -> Any:
    """
    ...
    if "tabu" in params and params["tabu"] is not None:
        ...
        if "noinc" in params and params["noinc"] is not None:
            if not isinstance(params["noinc"], int) or isinstance(
                params["noinc"], bool
            ):
...
def _validate_tree_params(params: Any, knowledge: Any, context: Any) -> Any:
    """
    ...
    if "tree" in params:
        tree = params["tree"]
        if (
            not isinstance(tree, tuple)
            or len(tree) != 3
            or any(
                [not isinstance(i, int) or isinstance(i, bool) for i in tree]
            )
        ):
...
def _validate_hc_arguments(
    ...
    if (
        ...
        or (knowledge is not False and not isinstance(knowledge, Knowledge))
    ...
    if (
        ...
        or (
            data.dstype == "categorical"
            and params["score"] in {"bic-g", "bge"}
        )
        or (
            data.dstype == "continuous"
            and params["score"] not in {"bic-g", "bge"}
        )
    ):
    ...
    if (
        knowledge is not False
        and knowledge.initial is not None
        and len(set(knowledge.initial.nodes) - set(data.get_order()))
    ):
        raise ValueError("hc() initial DAG contains unknown nodes")

    # Check context keys all valid

    if context is not None and len(
        set(context.keys())
...
def reorder_list(strings: Any, num_parts: int) -> list:
    """
...
def set_stable_order(data: Any, params: Any) -> tuple:
    """
    ...
    def _node_order(parents: Any, current: Any) -> tuple:
        _partial = SDG.partial_order(parents)
        assert _partial is not None
        return tuple([n for g in _partial for n in current if n in g])
    ...
    num_keys = (
        1
        if _params["stable"] == Stability.DEC_1
        else (2 if _params["stable"] == Stability.DEC_2 else 3)
    )
    ...
    if _params["stable"] not in {
        ...
        # that results in the graph with the best score
            ...
            for order in orders:
                ...
                if best is None or (
                    hcw.score > best[1]
                    and not values_same(hcw.score, best[1], sf=10)
                ):
                    best = (order, hcw.score, hcw.parents)
                print(
                    (
                        "Order {}, {} ... {}, {}"
                        " gives score {:.3f} after {:.3f}"
                    ).format(
                        order[0],
                        order[1],
                        order[-2],
    ...
    elapsed = time() - start
    print(
        "Stable {} order: {} in {:.3f}s".format(
            params["stable"].value, _order, elapsed
        )
    )
    return (data, elapsed)


def _score_order(data: Any, params: Any, num_keys: int = 3) -> Any:
    """
    ...
    for node in data.get_order():
        ...
        c_score = mean(
            [
                HCWorker.nscore(node, set([p]), data, params)[0] / N
                for p in data.get_order()
                if node != p
            ]
        )
        ...
        if order is None:
            ...
            for i, entry in enumerate(order):
                ...
                same_c_score = num_keys < 2 or values_same(
                    c_score, entry[1], sf=10
                )
                ...
                if (
                    ...
                    vals_same = (
                        ...
                        or (
                            ...
                            == data.as_df()[entry[2][0]].to_numpy()
                        ).all()
                    )
                    if not vals_same:
                        assert nvs is not None
                        v_n = "{}".format(
                            {v: nvs[node][v] for v in sorted(nvs[node])}
                        )
                        v_e = "{}".format(
                            {
                                v: nvs[entry[2][0]][v]
                ...
                if same_score and same_c_score and vals_same:
                    print(
                        "*** Variable {} and {} same position".format(
                            node, order[i][2][0]
                        )
                    )
                    ...
                    (not same_score and score > entry[0])
                    or (same_score and c_score > entry[1])
                    or (same_score and same_c_score and vals_after)
                ):
    ...
    order = tuple([n for e in order for n in e[2]])  # type: ignore[assignment]
    print("\n\nDecreasing score order is {}".format(order))

    return order


def hc_prune(hcws: list, params: Any, stats: Any) -> list:
    """
    ...
    if lookahead > 0:
        for hcw in hcws:
            ...
            print(
                "+++ lookahead {} score for {} is {:.4f} cf {:.4f}".format(
                    lookahead,
                    TreeStats.key(hcw),
                    hcw.score2 / hcw.data.N,
                    hcw.score / hcw.data.N,
                )
            )
...
def hc_subtree(
    ...
    if root is None:
        ...
        _know = Knowledge(
            rules=RuleSet.EQUIV_SEQ, params={"sequence": (False,)}
        )
    ...
    while iseq < 2**depth:
        ...
        sequence = tuple(
            root_seq
            + [True if (iseq >> s) & 1 == 1 else False for s in range(depth)][
                ::-1
            ]
        )
        ...
        if hcw.paused is False:
            decision_list = [
                k[1]
                for k in hcw.trace.trace["knowledge"]
                if k is not None and k[0] == Rule.EQUIV_SEQ.value
            ]
            decisions = sum(d is True for d in decision_list)
...
def hc_tree(data: Any, params: Any, context: Any, init_cache: bool) -> Any:
    """
    ...
    hcws: list[HCWorker | None] = [None]  # list of running HCWorkers
    ...
    while finished is False:
        ...
        for root in hcws:
            if root is None or root.paused is True:
                new_hcws += hc_subtree(
                    root, data, params, context, stats, init_cache
                )
            else:
                new_hcws += [root]
        hcws = new_hcws
        ...
        finished = all(
            [
                hcw.paused is False  # type: ignore[union-attr]
                or len(hcw.knowledge.sequence) >= 60  # type: ignore
                for hcw in hcws
            ]
        )

    assert hcws[0] is not None
    hcws[0].trace.set_treestats(stats)  # type: ignore[union-attr]
    return hcws[0]
...
def hc(
    ...
    arcs = [
        (c, "->", p) for p, children in hcw.parents.items() for c in children
    ]
    dag = DAG(list(hcw.parents.keys()), arcs)
    return (
        (dag, None) if hcw.trace is None else (dag, hcw.trace.set_result(dag))
    )
