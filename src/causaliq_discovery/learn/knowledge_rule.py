...
from causaliq_core.utils import EnumWithAttrs, ln, values_same
...
class Rule(EnumWithAttrs):
    ...
    def get_seed(cls) -> int:
        cls._seed = getattr(cls, "_seed", 0)  # type: ignore[attr-defined]
        cls._seed = 1 if cls._seed == 100 else cls._seed + 1  # type: ignore
        return cls._seed  # type: ignore[attr-defined, no-any-return]
    ...
    def match(
        ...
        if self.value in {"equiv_add", "equiv_seq"}:
            ...
            return self.test_bic_unstable(
                best.activity, best.arc, knowledge.threshold, date, parents
            )

        elif self.value == "hi_lt5":
            return self.test_hi_lt5(
                best.activity, best.counts, knowledge.threshold
            )
            ...
            return self.test_lo_delta(
                best.delta, knowledge.max_abs_delta, knowledge.threshold
            )
        else:
            return None

    def test_equiv_add(self, best: Any, second: Any, sf: Any) -> Any:
        """
        ...
        return (
            ...
            and values_same(best.delta, second.delta, sf)
            else None
        )
    ...
    def test_mi_discrepancy(
        ...
        """
        if best.activity != GraphAction.ADD or values_same(
            best.delta, second.delta, sf
        ):
            return None
        ...
        N = date.N
        mi = round(
            indep(best.arc[0], best.arc[1], None, date.sample)["mi"][
                "statistic"
            ]
            / (2 * N),
            6,
        ...
        rel_mi = (
            ...
            if mi == 0.0
                ...
                1.0
                if delta == 0.0
                else min(max(1.0 + 0.1 * ln(mi / delta, 2), 0.0), 1.0)
            )
        )
    ...
    def test_bic_unstable(
        ...
        for node, parents in changed.items():
            ...
            ll_score = node_score(
                node, parents, ["loglik"], {"base": "e"}, data
            )
            ll: Any = ll_score["loglik"]  # type: ignore[call-overload]
            data.set_N(half, seed, random_selection=True)
            ll_h_score = node_score(
                node, parents, ["loglik"], {"base": "e"}, data
            )
            ll_h: Any = ll_h_score["loglik"]  # type: ignore[call-overload]
            data.set_N(N)
            instability = (
                abs(round((ll - 2 * ll_h) / ll, 3)) if ll < 0.0 else 0.0
            )
            print(
                "\n*** BIC INSTABILITY [{}]: {} {}: "
                "{:.3f} vs {:.3f} -> {}".format(
                    seed, node, parents, (ll - ll_h), ll_h, instability
                )
            )
    ...
    def test_hi_lt5(self, activity: Any, counts: Any, threshold: Any) -> Any:
        """
        ...
        return (
            Rule.HI_LT5
            if counts["lt5"] is not None and counts["lt5"] > threshold
            else None
        )

    def test_lo_delta(
        self, delta: Any, max_abs_delta: Any, threshold: Any
    ) -> Any:
        """
        ...
        return (
            ...
            and abs(delta) < max_abs_delta * threshold
            else None
        )

    def test_pos_delta(self, activity: Any, delta: Any) -> Any:
        """
...
class RuleSet(EnumWithAttrs):
    ...
    EQUIV_LT5 = (
        "equiv_lt5",
        "Equiv add and high LT5",
        [Rule.EQUIV_ADD, Rule.HI_LT5],
    )
    ...
    def __init__(
        self, _: str, label: str, rules: list[Any] | None = None
    ) -> None:
        self._label_ = label
        self._rules_ = rules
...
class KnowledgeEvent:
    ...
    def __init__(
        ...
        if (
            ...
            or not isinstance(arc, tuple)
            or len(arc) != 2
            or not isinstance(arc[0], str)
            or not isinstance(arc[1], str)
        ):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        return (
            isinstance(other, KnowledgeEvent)
    ...
    def __str__(self) -> str:
        ...
        return "{} ({}) {} {}".format(
            ...
            (
                ...
                else ("F" if self.correct is not None else "None")
            ),
            self.outcome,
            self.arc,
        )
