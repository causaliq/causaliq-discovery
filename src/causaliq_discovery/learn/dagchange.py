...
class DAGChange:
    ...
    def __init__(
        self,
        activity: GraphAction,
        arc: Optional[Tuple[str, str]] = None,
        delta: Optional[float] = None,
        counts: Optional[Dict[str, Any]] = None,
    ) -> None:
        if (
            not isinstance(activity, GraphAction)
            or (arc is not None and not isinstance(arc, tuple))
            or (delta is not None and not isinstance(delta, float))
            or (counts is not None and not isinstance(counts, dict))
        ):
    ...
    def __str__(self) -> str:
        ...
        """
        return (
            "{}, score={}".format(self.activity, self.delta)
            if self.activity in [GraphAction.INIT, GraphAction.STOP]
            else "{} {}, score={}, count={}".format(
                self.activity, self.arc, self.delta, self.counts
            )
        )
...
class BestDAGChanges:
    ...
    def __init__(
        self,
        top: Optional["DAGChange"] = DAGChange(GraphAction.STOP),
        second: Optional["DAGChange"] = None,
    ) -> None:
        ...
        self.top = (
            DAGChange(top.activity, top.arc, top.delta, top.counts)
            if top is not None
            else None
        )
        self.second = (
            DAGChange(second.activity, second.arc, second.delta, second.counts)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        return (
            isinstance(other, BestDAGChanges)
